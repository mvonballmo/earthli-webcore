...
function &custom_make_environment ()
{
  ...
  $Result->exception_handler_page = '/suite/handle_exception.php';

  /* Set the logger stylesheet used by HTML loggers. */

  $Result->logger_style_sheet = '/suite/common/styles/log.css';
...
function &custom_make_page (&$env)
{
  ...
  $Result->set_path (Folder_name_root, '/suite/');
  $Result->set_path (Folder_name_resources, '{root}/common/');
  $Result->set_path (Folder_name_pages, '{root}');
