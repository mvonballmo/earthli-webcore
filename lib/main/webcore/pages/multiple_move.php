...
****************************************************************************/
  ...
  if (isset ($folder))
  {
    ...
    /** @var MULTIPLE_OBJECT_MOVER_FORM $form */
  ...
  if (isset ($form))
  {
    ...
    $Page->title->subject = 'Move ' . $form->object_list->description ();

    $Page->location->add_folder_link ($folder);
    $Page->location->append ("Explorer", "view_explorer.php?id=$folder->id");
    $Page->location->append ($Page->title->subject, '', '{icons}buttons/move');
  ...
  {
    $Page->raise_security_violation ('You are not allowed to move from this folder.', $folder);
  }
?>