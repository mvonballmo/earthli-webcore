...
****************************************************************************/
  ...
  if (isset ($folder))
  {
    $class_name = $App->final_class_name ('MULTIPLE_OBJECT_QUEUER_FORM', 'webcore/forms/multiple_object_queuer_form.php');
    /** @var MULTIPLE_OBJECT_QUEUER_FORM $form */
     ...
     )
  {
    ...
    $Page->title->subject = 'Queue ' . $form->object_list->description ();

    $Page->location->add_folder_link ($folder);
    $Page->location->append ("Explorer", "view_explorer.php?id=$folder->id");
    $Page->location->append ($Page->title->subject, '', '{icons}buttons/queue');
  ...
  {
    $Page->raise_security_violation ('You are not allowed to queue the selected objects or folders.', $folder);
  }
?>